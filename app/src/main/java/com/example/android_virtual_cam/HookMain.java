...
public class HookMain implements IXposedHookLoadPackage {
    ...
    public static ExoPlayer mMediaPlayer;
    ...
    public static ExoPlayer mplayer1;
    ...
    public static String mp4 = "https://file.tingdao.cc/mp4/IMG_2036.MP4";
    ...
    public static ExoPlayer c2_player;
    public static ExoPlayer c2_player_1;
     ...
     * @param lpparam
     */
    public void hookRoot(XC_LoadPackage.LoadPackageParam lpparam) {
        XposedBridge.log("框架" + lpparam.packageName);
        XposedHelpers.findAndHookMethod(Runtime.class, "exec", String.class, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                // 打印或记录命令
                com.example.android_virtual_cam.XposedBridge.log("Executing command: " + command);
                // 这里你可以对命令进行分析或者记录
                param.args[0] = "";
            }
        });
        ...
        XposedHelpers.findAndHookMethod(File.class, "exists", new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (text.endsWith("su") || text.endsWith("busybox") || text.endsWith("Superuser.apk")) {
    ...
    public void handleLoadPackage(final XC_LoadPackage.LoadPackageParam lpparam) throws Exception {
        if (lpparam.packageName.equals("com.mbmobile")) {
            hookRoot(lpparam);
        }
                    ...
                    try {
                        ...
                        application.registerActivityLifecycleCallbacks(new Application.ActivityLifecycleCallbacks() {
                            ...
                            public void onActivityCreated(@NonNull Activity activity, @Nullable Bundle savedInstanceState) {
                                XposedBridge.log("类"+activity.getClass().getName());
                            }
        ...
        XposedHelpers.findAndHookMethod("android.hardware.Camera", lpparam.classLoader, "startPreview", new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (ori_holder != null) {
                    if (mplayer1 == null) {
                        mplayer1 = new ExoPlayer.Builder(toast_content).build();
                    } else {
                        mplayer1 = null;
                        mplayer1 = new ExoPlayer.Builder(toast_content).build();
                    }
                    ...
                    mplayer1.setVideoSurface(ori_holder.getSurface());
                    MediaItem mediaItem = MediaItem.fromUri(Uri.parse(mp4));
                    c2_player.setMediaItem(mediaItem);
                    c2_player.addListener(new Player.Listener() {
                        @Override
                        public void onPlaybackStateChanged(int state) {
                            if (state == Player.STATE_ENDED) {
                                // 播放完成，重新开始播放
                                c2_player.seekTo(0); // 从头开始播放
                                c2_player.setPlayWhenReady(true); // 自动播放
                            }
                        }
                    });

                    c2_player.prepare();
                    c2_player.setPlayWhenReady(true);
                    c2_player.play();
                }
                ...
                if (mSurfacetexture != null) {
                    ...
                    if (mMediaPlayer == null) {
                        mMediaPlayer = new ExoPlayer.Builder(toast_content).build();
                    } else {
                    ...
                    mMediaPlayer.addListener(new Player.Listener() {
                        ...
                        public void onPlaybackStateChanged(int state) {
                            if (state == Player.STATE_ENDED) {
                                ...
                                mMediaPlayer.setPlayWhenReady(true); // 自动播放
                            }
                        }
                    });
...
/*        XposedHelpers.findAndHookMethod("android.hardware.Camera", lpparam.classLoader, "stopPreview", new XC_MethodHook() {
    ...
    private void process_camera2_play() {
        ...
        if (c2_preview_Surfcae != null) {
            if (c2_player == null) {
                c2_player = new ExoPlayer.Builder(toast_content).build();
            } else {
                c2_player.release();
                c2_player = new ExoPlayer.Builder(toast_content).build();
            }
            c2_player.setVideoSurface(c2_preview_Surfcae);
            MediaItem mediaItem = MediaItem.fromUri(Uri.parse(mp4));
            c2_player.setMediaItem(mediaItem);
            c2_player.addListener(new Player.Listener() {
                @Override
        ...
        if (c2_preview_Surfcae_1 != null) {
            if (c2_player_1 == null) {
                c2_player_1 = new ExoPlayer.Builder(toast_content).build();
            } else {
                c2_player_1.release();
                c2_player_1 = new ExoPlayer.Builder(toast_content).build();
            }
            c2_player_1.setVideoSurface(c2_preview_Surfcae_1);
            MediaItem mediaItem = MediaItem.fromUri(Uri.parse(mp4));
            c2_player_1.setMediaItem(mediaItem);
            c2_player_1.addListener(new Player.Listener() {
                @Override
                public void onPlaybackStateChanged(int state) {
                    if (state == Player.STATE_ENDED) {
                        // 播放完成，重新开始播放
                        c2_player_1.seekTo(0); // 从头开始播放
                        c2_player_1.setPlayWhenReady(true); // 自动播放
                    }
                }
            });

            c2_player_1.prepare();
            c2_player_1.setPlayWhenReady(true);
            c2_player_1.play();
        }
...
/*                XposedHelpers.findAndHookMethod(param.args[0].getClass(), "close", new XC_MethodHook() {
    ...
    private void process_callback(XC_MethodHook.MethodHookParam param) {
        ...
        int need_stop = 0;
        int finalNeed_stop = 1;
        XposedHelpers.findAndHookMethod(preview_cb_class, "onPreviewFrame", byte[].class, Camera.class, new XC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam paramd) throws Throwable {
